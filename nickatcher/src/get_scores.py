...
async def get_scores(slskd_client: SLSKDClient, session: AsyncSession, lda: LDA, dist: np.ndarray, room_name: str, max_tokens: int, min_chunks:int, user_1: str, user_2: str):
  ...
  user_embeddings_1, num_tokens_1 = await asyncio.to_thread(get_embeddings, group_messages_1, max_tokens=max_tokens)
  user_embeddings_2, num_tokens_2 = await asyncio.to_thread(get_embeddings, group_messages_2, max_tokens=max_tokens)
