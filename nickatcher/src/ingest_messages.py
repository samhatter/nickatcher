            ...
            if messages:
                for message in sorted(messages, key=lambda message: message['timestamp']):
                    ...
                    if not last_user_message or timestamp > last_user_message.timestamp:
                        ...
                        await handle_commands(slskd_client=slskd_client, session=session, lda=lda, dist=dist, max_tokens=max_tokens, min_chunks=min_chunks, room_name=room_name, user=message['username'], text=message['content'])
